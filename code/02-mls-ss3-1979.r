...
# resolve NZ rec selex (share terminal bins)
# tail compression
# extra obs se cpue index
# estimate end logit parameter for selex
# downweight size comp
...
# create new directory for stock synthesis mls files
    dir_mls_stock_synthesis_base = paste0(dir_model,"stock-synthesis/02-mls-ss3-1979/")
...
# update control
    ...
    tmp_initF[,"LO"] = 0
    tmp_initF[,"HI"] = 5
    tmp_initF[,"INIT"] = tail(as.matrix(tmp_ctl$natM)[1,],n=1)/4 * 1.7
    tmp_initF[,"PRIOR"] = 0.2
    ...
    # extra obs for cpue
    loess_y = tmp_cpue$obs
    loess_x = tmp_cpue$year

    loess_cpue = loess(loess_y~loess_x)
    

    tmp_ctl$Variance_adjustment_list$Value[intersect(which(tmp_ctl$Variance_adjustment_list$Factor%in%c(1)),which(tmp_ctl$Variance_adjustment_list$Fleet%in%c(15)))] = mean(abs(loess_cpue$residuals))

    # adjust ess for comps
    length_var_adj = 1/c(10,12,10,10,10,18,10,10,10,10,16,34,10,10,15)
    weight_var_adj = 1/c(10,10,10,10,10,10,10,10,12,23,10,10,10,10,15)
