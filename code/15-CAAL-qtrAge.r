...
# transfer files
    ...
    to_dir = file.path(dir_model,"stock-synthesis","15-CAAL-qtrAge")
...
# convert to eye orbital fork length in cm
    ...
    age_dt = fread(file.path(proj_dir,"data","MLS age data for SPC_04112023.csv")) %>%
            ...
            .[,caal_age_bin:=floor(age/0.25)*0.25] %>%
            ...
            .[,age:=paste0("age",age)] %>%
            .[,age:=factor(age,levels=paste0("age",seq(from=0,to=tmp_data$Nages,by=0.25)))] %>%
...
# integer
tmp_data$N_agebins = ncol(caal_data)
# vector of ages
tmp_data$agebin_vector = seq(from=0,to=tmp_data$Nages,by=0.25)
# integer
...
colnames(ageerror_mat) = paste0("age",seq(from=0,to=tmp_data$Nages,by=0.25))
ageerror_mat[1,] = seq(from=0,to=tmp_data$Nages,by=0.25) + 0.125
ageerror_mat[2,] = round(seq(from=0.01,to=0.01,length.out=ncol(ageerror_mat)),digits=2)
