...
# 2025/01/23
# Sensitivity to excluding high spike in CPUE
...
# transfer files
    ...
    to_dir = file.path(dir_model,"stock-synthesis","16-CAAL-rm-spike")
...
#________________________________________________________________________________________________________________________________________________________________________________________________________
# modify files; data
# remove 1998 spike in CPUE
        tmp_cpue = as.data.table(tmp_data$CPUE) %>%
                  .[year==1998,index:=abs(index)*-1]   
        tmp_data$CPUE = as.data.frame(tmp_cpue)
