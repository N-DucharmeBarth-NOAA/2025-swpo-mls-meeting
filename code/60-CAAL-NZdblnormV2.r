...
for(i in 1:length(selex_vec)){
    ...
    # transfer files
        ...
        to_dir = file.path(dir_model,"stock-synthesis",paste0(59+i,"-CAAL-NZdblnormV2-",gsub("0.","",as.character(selex_vec[i]),fixed=TRUE)))
    ...
    pointer = grep("SizeSel_P_6",rownames(tmp_ctl$size_selex_parms),fixed=TRUE)
    tmp_ctl$size_selex_parms$INIT[pointer] = -999
    tmp_ctl$size_selex_parms$PRIOR[pointer] = 0
    tmp_ctl$size_selex_parms$PHASE[pointer] = -3
