...
# 2025/04/07
# Update max age to 15 (max decimal age is 15.5 but floor is applied to CAAL)
...
# transfer files
    ...
    to_dir = file.path(dir_model,"stock-synthesis","26-CAAL-maxAge-15")
...
# integer
tmp_data$N_agebins = tmp_data$Nages + 1
# vector of ages
...
ageerror_mat[2,] = c(round(seq(from=0.35,to=2,length.out=11),digits=2),rep(2,5))
...
# modify files; ctl
    ...
    tmp_M = matrix(c(as.vector(as.matrix(tmp_ctl$natM)), rep(tail(as.vector(as.matrix(tmp_ctl$natM)),n=1),5)),nrow=1,ncol=tmp_data$Nages+1)
    colnames(tmp_M) = paste0("Age_",seq(from=0,to=tmp_data$Nages))
