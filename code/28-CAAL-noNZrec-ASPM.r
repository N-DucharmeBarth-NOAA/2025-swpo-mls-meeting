...
# transfer files
    ...
    to_dir = file.path(dir_model,"stock-synthesis","28-CAAL-noNZrec-ASPM")
...
# modify files; ctl
# fix selex for fleet 
    pointer = grep("F10",rownames(tmp_ctl$size_selex_parms),fixed=TRUE)
    tmp_ctl$size_selex_parms$PHASE[pointer] = -1 * abs(tmp_ctl$size_selex_parms$PHASE[pointer])
    
# lambda for fleet 10 to 0
    tmp_lambdas_surv = as.data.table(tmp_ctl$lambdas)

    tmp_lambdas_gs = data.table(like_comp=6,fleet=10) %>%
                      .[,phase:=1] %>%
                      .[,value:=0] %>%
                      .[,sizefreq_method:=1]
