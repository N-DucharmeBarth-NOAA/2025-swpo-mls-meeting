...
# 2025/04/07
# only estimate selex for one fleet at a time
...
# define OS
    ...
    fleet_vec = c(2,3,6,7,9,10,11)

for(i in 1:length(fleet_vec)){
    #________________________________________________________________________________________________________________________________________________________________________________________________________
    # transfer files
        from_dir = dir_base_stock_synthesis
        if(fleet_vec[i]<10){
            ii = paste0("0",fleet_vec[i])
        } else {
            ii = fleet_vec[i]
        }
        to_dir = file.path(dir_model,"stock-synthesis",paste0(46+i,"-CAAL-oneFleet-",ii))
    ...
    # fix selex
        ...
        freeze_idx = which(unname(sapply(rownames(tmp_ctl$size_selex_parms),function(x) if(length(grep(paste0("F",ii),x,fixed=TRUE))>0){1}else{0}))!=1)
        tmp_ctl$size_selex_parms$PHASE[freeze_idx] = -1 * abs(tmp_ctl$size_selex_parms$PHASE[freeze_idx])
    ...
    # lambda for fleets
        ...
        tmp_ctl$lambdas = rbind(tmp_lambdas_surv,tmp_lambdas_gs1,tmp_lambdas_gs2) %>%
                          .[fleet==fleet_vec[i],value:=1] %>%
                          as.data.frame(.)
